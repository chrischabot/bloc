...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < 5; i++) {
    await h.app.request('http://t/v1/search', { method: 'POST', headers, body: queryBody });
  }
  ...
  for (let i = 0; i < iters; i++) {
    ...
    const res = await h.app.request('http://t/v1/search', {
      method: 'POST',
      headers,
      body: queryBody,
    });
  ...
  const report = {
    ...
    errors: 0,
    budget_p99_ms: 200,
    passed: percentile(samples, 99) < 200,
    host: {
      hostname: hostname(),
  ...
  writeFileSync(join(REPORT_DIR, `phase-5-${stamp}.json`), JSON.stringify(report, null, 2));
  console.log(
    `[bench-phase-5] p50=${report.p50_ms}ms p90=${report.p90_ms}ms p99=${report.p99_ms}ms passed=${report.passed}`,
  );
...
void main().catch((err: unknown) => {
  console.error('[bench-phase-5] failed', err);
  process.exit(1);
});
